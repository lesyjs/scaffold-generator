...
module.exports = class Scaffold extends EventEmitter {
  constructor(options) {
    ...
    const { render, override = true, backup = true, data, ignore } = options;
  ...
  async _copyFileToFile(fromFile, to) {
    const toFile = this._to(slash(to));
  ...
  _shouldOverride(file) {
    ...
    return fs.exists(file).then((exists) => {
      ...
      return fs.copy(file, backFile).then(() => true);
    });
  }
};
